...
★ BRING YOUR OWN KEY
Gist uses YOUR own OpenAI, Anthropic, or Google Gemini API key. That means:
• No subscription and no markup — you pay the AI provider directly, at cost.
• Choose your model (GPT‑5.x or Claude, or Gemini) and trade off speed vs. quality.
...
★ HOW TO START
1. Install Gist.
2. Open Settings and paste your OpenAI, Anthropic, or Google Gemini API key.
...
★ 본인 API 키 사용
Gist는 사용자 본인의 OpenAI · Anthropic · Google Gemini API 키로 동작합니다.
• 구독·중간 마진 없음 — AI 제공자에 원가로 직접 지불
• 모델 선택 가능(GPT‑5.x / Claude / Gemini)으로 속도와 품질을 조절
...
★ 시작하기
1. Gist 설치
2. 설정에서 OpenAI · Anthropic · Google Gemini API 키 입력
...
summarize or translate is sent directly from your browser to OpenAI, Anthropic, or Google using
...
- host_permissions(api.openai.com, api.anthropic.com, generativelanguage.googleapis.com): 사용자 키로 AI API 호출
